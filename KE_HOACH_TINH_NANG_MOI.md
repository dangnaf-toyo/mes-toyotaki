...
- [x] **Quản lý danh mục tập trung** (2026-08-14) — đã code xong, CHƯA triển khai/test. Thêm policy ghi (insert/update/delete, giới hạn role admin qua `has_role()`) cho `master_products`/`master_machines`/`master_employees`/`duc_shot_khuon` (`supabase/migration_phase_S3_danh_muc_write.sql` — cần chạy trong SQL Editor), trang `quan-ly-danh-muc.html` (4 tab: Máy/Sản phẩm/Nhân sự/Khuôn, chỉ admin vào được, thêm/sửa/xoá). Không đổi các RPC ghi hiện có (`duc_end_shift`...) vì chạy `security definer`, bỏ qua RLS.
