...
- [ ] **Quản lý tài khoản & phân quyền** — hiện ai đăng nhập (Supabase Auth) cũng ghi được ở MỌI module. Cần: (a) trang admin tạo/xem danh sách tài khoản (không cần vào Supabase Dashboard thủ công), (b) gán vai trò (trưởng ca/IPQC/QC manager/kho NVL/kế hoạch/admin), (c) RLS/RPC kiểm tra vai trò trước khi cho ghi (hiện RLS chỉ kiểm tra `auth.role() = 'authenticated'`, chưa phân biệt vai trò cụ thể).
