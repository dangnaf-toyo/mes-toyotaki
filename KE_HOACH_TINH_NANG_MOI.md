...
- [ ] **Routing/BOM theo mã sản phẩm và phiên bản** — thay vì áp một tuyến cố định, mỗi mã SP cần có tuyến công nghệ được phê duyệt: đúc → bavia → CNC → sơn → OQC; đồng thời hỗ trợ nhánh insert, mạ/in thuê ngoài, rửa, kiểm tra rò khí và lắp ráp. Cần có ngày hiệu lực/phiên bản để thay đổi quy trình không làm sai lịch sử.
...
- [ ] **Sổ đời khuôn đúc và khuôn dập gate** — mở rộng quản lý từ số shot sang toàn bộ vòng đời: khuôn/mã SP áp dụng, số shot, shot nóng, ngưỡng bảo dưỡng, lịch sử sửa chữa, chi phí, thời gian dừng máy, lỗi liên quan và quyết định thay mới. Khuôn dập gate cần được quản lý riêng vì cũng chịu khấu hao.
