// Edge Function: get-document-source-url
// Trả về signed URL ngắn hạn (60 giây) để TẢI bản gốc CAD của 1 phiên bản tài
// liệu kỹ thuật, sau khi tự kiểm tra quyền 'download_source' của người gọi
// qua RPC has_doc_permission (bucket 'tech-documents' là PRIVATE, client
// không được gọi thẳng storage.from()). Mọi lượt tải/lượt bị chặn đều được
// ghi vào document_access_log để phục vụ audit.
//
// Deploy: supabase functions deploy get-document-source-url
// Gọi từ trang tĩnh: POST {SUPABASE_URL}/functions/v1/get-document-source-url
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: versionRow, error: versionErr } = await admin
      ...
      .select('id, document_id, file_path_goc')
      .eq('id', versionId)
      .maybeSingle();
    ...
    if (!allowed) {
      ...
      return json({ error: 'Bạn không có quyền tải bản gốc CAD của tài liệu này' }, 403);
    }

    if (!versionRow.file_path_goc) {
      return json({ error: 'Tài liệu này không có bản gốc CAD' }, 404);
    }

    const { data: signed, error: signErr } = await admin.storage
      .from('tech-documents')
      .createSignedUrl(versionRow.file_path_goc, 60);
    if (signErr || !signed?.signedUrl) {
      return json({ error: 'Không tạo được đường dẫn tải: ' + (signErr?.message || '') }, 500);
    }
