-- ============================================================================
-- Phase T27 — Bavia: chống double-submit (R1) + siết vài chỗ nhỏ
--
--   - bavia_xu_ly.client_key / bavia_sua_log.client_key  (unique, nullable)
--   - bavia_xu_ly_thung(+ p_client_key) : gọi lại cùng key → trả kết quả cũ,
--     KHÔNG tạo sự kiện / trừ nguồn lần 2. Chèn header bavia_xu_ly LÊN ĐẦU +
--     on conflict do nothing để lối thoát sạch.
--   - bavia_sua_hang(+ p_client_key)    : guard bằng SELECT + unique index.
--
-- R3 (kiểm ma_sp thùng gộp) đã xử lý ở T26. R2 (undo) đã có ở T26/GĐ4.
--
...
-- Thêm tham số mới → phải DROP bản 11 tham số (T24/T26) tránh nhập nhằng.
drop function if exists bavia_xu_ly_thung(text, text[], numeric, numeric, numeric, text, text, numeric, text, text, text);
...
declare
  ...
  v_avail  numeric := 0;
  v_n      int;
  v_rem numeric[]; v_lay numeric[]; v_phe_i numeric[]; v_sua_i numeric[];
  v_ngay_moi date;
  v_lot text[] := '{}'; v_khu text[] := '{}'; v_nl text[] := '{}'; v_may text[] := '{}';
  v_ma_sp text; v_ten_sp text;
  v_pos text;
  v_out_qty numeric[] := '{}'; v_out_tag text[] := '{}';
  v_id_xu_ly bigint;
  ...
  v_ng_sua_tag text; v_gop_ma text;
begin
  ...
  -- ── Chống double-submit: header LÊN ĐẦU, key trùng thì trả kết quả cũ ──
  insert into bavia_xu_ly (id_tram, ten_tram, ngay, ca, cong_doan, ma_sp, tong_ok, tong_ng_phe, tong_ng_sua, ng_ly_do, nguoi, client_key)
  values (p_id_tram, v_tram.ten_tram, v_tram.ngay, v_tram.ca, v_tram.cong_doan, v_tram.ma_sp, v_ok, v_phe, v_sua,
          nullif(trim(p_ng_ly_do), ''), coalesce(nullif(trim(p_nguoi), ''), p_user), nullif(trim(p_client_key), ''))
  on conflict (client_key) do nothing
  returning id into v_id_xu_ly;

  if v_id_xu_ly is null then
    select id into v_id_xu_ly from bavia_xu_ly where client_key = nullif(trim(p_client_key), '');
    select coalesce(jsonb_agg(jsonb_build_object(
        'tag', r.tag_moi,
        'so_luong', (select d.so_luong from duc_tem d where d.tag_no = r.tag_moi),
        'loai', case when r.loai = 'ok_gop' then 'ok_gop'
                     when exists(select 1 from duc_tem d where d.tag_no = r.tag_moi and d.trang_thai = 'NG chờ sửa') then 'ng_sua'
                     else 'ok' end)), '[]'::jsonb)
      into v_tems from bavia_xu_ly_ra r where r.id_xu_ly = v_id_xu_ly;
    return jsonb_build_object('ok', true, 'id_xu_ly', v_id_xu_ly, 'tems', v_tems, 'duplicate', true, 'tong_lay', v_can);
  end if;

  v_n := array_length(p_tag_nguon, 1);
  v_rem := array_fill(0::numeric, array[v_n]); v_lay := array_fill(0::numeric, array[v_n]);
  v_phe_i := array_fill(0::numeric, array[v_n]); v_sua_i := array_fill(0::numeric, array[v_n]);
  ...
  for i in 1 .. v_n loop
    ...
    v_rem[i] := v_src.so_luong; v_avail := v_avail + v_src.so_luong;
  ...
  v_static_lot := array_to_string(v_lot, ', '); v_static_khu := array_to_string(v_khu, ', ');
  v_static_nl := array_to_string(v_nl, ', '); v_static_may := array_to_string(v_may, ', ');
  ...
  if v_ok > 0 then
    if p_cach_dong = 'gop' then
      ...
      v_out_qty := array[v_ok]; v_out_tag := array[p_tag_gop];
      ...
      while v_need > 0 loop
        ...
        v_out_qty := array_append(v_out_qty, v_q); v_out_tag := array_append(v_out_tag, bavia_next_tag_no());
        v_need := v_need - v_q;
      end loop;
    else
      v_out_qty := array[v_ok]; v_out_tag := array[bavia_next_tag_no()];
    end if;
  ...
  if v_sua > 0 then
    v_tag := v_ng_sua_tag; v_need := v_sua;
    while v_need > 0 loop
  ...
  return jsonb_build_object('ok', true, 'id_xu_ly', v_id_xu_ly, 'tems', v_tems, 'tong_lay', v_can);
end;
$$;
revoke execute on function bavia_xu_ly_thung(text, text[], numeric, numeric, numeric, text, text, numeric, text, text, text, text) from anon;
grant  execute on function bavia_xu_ly_thung(text, text[], numeric, numeric, numeric, text, text, numeric, text, text, text, text) to authenticated;

-- ── bavia_sua_hang — + p_client_key (guard SELECT + unique index) ─────────
drop function if exists bavia_sua_hang(text, date, text, text, numeric, numeric, text, text, numeric, text, text, text);
...
begin
  ...
  -- dedup: gọi lại cùng key → trả kết quả cũ
  ...
  insert into bavia_sua_log (tag_ng, ngay, ca, cong_doan, ma_sp, so_ok, so_ng_phe, ly_do, nguoi, tag_ra, client_key)
