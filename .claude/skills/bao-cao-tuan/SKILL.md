...
- **So sánh Ca 1 và Ca 2**: gộp toàn bộ 5-6 ca mỗi bên theo nhãn `ca` (không theo `truong_ca` — 2 trường này thường khớp 1-1 nhưng không tuyệt đối, xem mục đổi khuôn bên dưới). Các chỉ số: Hoàn thành KH, Phút dừng máy, Số sự cố/ca (trung bình), NG. **Không có hàng Availability/Performance/Quality/OEE** — nhà máy chưa quản lý OEE.
- **Thời gian đổi khuôn theo trưởng ca**: lọc `duc_su_co_log` có `loai_su_co` bắt đầu bằng "C3" (đổi khuôn, cần chuẩn hoá dấu gạch ngang — xem mục 4), **gộp theo trường `truong_ca` (người ghi nhận), không theo nhãn `ca`** — đây là điểm khác biệt duy nhất so với các phân tích khác, vì user hỏi "so sánh ca Liêm/ca Mẫn" nghĩa là so theo người, và thực tế có vài bản ghi `truong_ca` không khớp `ca` (vd người trực Ca 1 nhưng có sự cố ghi nhận muộn dưới nhãn Ca 2). Tính: số lần, tổng phút, trung bình phút/lần cho mỗi trưởng ca xuất hiện trong tuần. Liệt kê đầy đủ từng lần trong bảng chi tiết (ngày/máy/nội dung/phút).
...
3. **Giữ nguyên thứ tự các section**: Header → KPI strip → Diễn biến theo ngày (+ 3 ca yếu nhất) → Sản lượng theo mã SP → Sản lượng theo máy → So sánh Ca 1/Ca 2 (+ ưu tiên) → Thời gian đổi khuôn theo trưởng ca → Thời gian dừng theo máy (+ ưu tiên) → Thời gian dừng theo loại vấn đề (+ ưu tiên) → Thời gian dừng theo mã SP (+ ưu tiên) → 3 vấn đề lớn nhất (theo máy / theo loại / theo mã SP, dạng card 3 cột) → Footer ghi chú nguồn số liệu.
